    ...
    def configure_mesh_path(self) -> None:
        ...
        for link in self.robot.links:
            ...
            for visual in link.visuals:
                if(os.path.isabs(visual.geometry.filename)):
                    visual.geometry.filename = visual.geometry.filename  # if urdf meshes use absolute path
                else:  # if urdf meshes use relative path
                    urdf_abs_path = os.path.dirname(self.file_path)
                    abs_path = os.path.abspath(os.path.join(urdf_abs_path, visual.geometry.filename))
